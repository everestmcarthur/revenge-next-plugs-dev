...
export function BetterTableRowTitle({
	...
	const UseComponent = onPress ? RN.Pressable : RN.View
...
export function BetterTableRowGroup({
	...
	const styles = stylesheet.createThemedStyleSheet({
		main: {
			backgroundColor:
				(semanticColors as any).CARD_BACKGROUND_DEFAULT ?? '#1e1f22',
			borderColor: (semanticColors as any).BORDER_MUTED ?? '#2b2d31',
			borderWidth: 1,
			borderRadius: 16,
			overflow: 'hidden',
			flex: 1,
