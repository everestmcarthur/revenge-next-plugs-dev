import { DocumentPicker, DocumentsNew, RNFileModule } from './nativeModules'
...
export async function saveFile(name: string, content: any) {
	if (!DocumentsNew || !RNFileModule) return false

	const filename = `tmp_cloudsync_${name}`
	const path = await RNFileModule.writeFile('cache', filename, content, 'utf8')
	return await DocumentsNew.saveDocuments({
		...
		.then((x: any) => x[0])
		.finally(() => void RNFileModule.removeFile('cache', filename))
}
