...
export async function loginWithUserId(userId: string): Promise<string> {
	const res = await fetch(`${constants.api}api/auth/token?userId=${userId}`)
...
export function rawDataURL() {
	return `${constants.api}api/data/raw?auth=${encodeURIComponent(
		useAuthorizationStore.getState().token ?? '',
	)}`
}
