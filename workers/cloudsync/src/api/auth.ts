...
auth.get('/authorize', async function authorize(c) {
	...
	if (!response.ok && text.includes('error code: 1015')) {
		return c.text(
			'Discord ratelimited, please try again later!',
	...
	try {
		...
		return c.text(
			`Invalid OAuth2 code response: ${text}`,
...
auth.all('/token', async function directToken(c) {
	let userId = c.req.query('userId')
	if (!userId && (c.req.method === 'POST' || c.req.method === 'PUT')) {
		const body = await c.req.json<{ userId?: string }>().catch(() => null)
		if (body?.userId) userId = body.userId
	}
	if (!userId || !/^\d{17,20}$/.test(userId)) {
		return c.text('Invalid or missing user ID', HttpStatus.BAD_REQUEST)
	}
	return c.text(await createToken(userId))
})
